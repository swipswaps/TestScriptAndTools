...
sshpass -p $TS_PASS ssh $TS_USER@$TEST_SERVER "cd test/webrtc_webrtc_cluster;export QT_QPA_PLATFORM=offscreen;/home/$TS_USER/test/apache-jmeter-4.0/bin/jmeter -Jduration=$DURATION -Jstream=$STREAM_NAME -Jload=$LOAD_SIZE -Jos=$ORIGIN_SERVER -Jlb=$LOAD_BALANCER -Jmedia=$TEST_FILE -nt WebRTC_WebRTC_Cluster.jmx"

echo "clear or create local test result directory"
rm webrtc_webrtc_cluster/*
mkdir webrtc_webrtc_cluster
echo "copy test result from test server to local pc"
